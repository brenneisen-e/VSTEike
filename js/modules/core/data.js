/**
 * Data Management Module - ES2024
 * State management, filtering, and aggregation
 */
...
export let dailyRawData = null;

export const setDailyRawData = (data) => {
    dailyRawData = data;
    window.dailyRawData = data;
};
...
export const aggregateDailyToMonthly = (dailyData) => {
    const monthlyData = {};
...
export const getAgenturen = () => {
    if (!dailyRawData?.length) return [];
...
export const getAgenturData = (agenturId) => {
    if (!dailyRawData?.length) return null;

    let agenturData = dailyRawData.filter(row => row.vermittler_id == agenturId);

    const year = state.filters.year;
    if (agenturData[0]?.year !== undefined) {
        agenturData = agenturData.filter(row => row.year == year);
    }

    if (state.filters.silo !== 'alle' && agenturData[0]?.silo !== undefined) {
        agenturData = agenturData.filter(row => row.silo === state.filters.silo);
    }

    if (!state.filters.segments.includes('alle') && agenturData[0]?.segment !== undefined) {
        agenturData = agenturData.filter(row => state.filters.segments.includes(row.segment));
    }

    if (!state.filters.products.includes('alle') && agenturData[0]?.product !== undefined) {
...
export const getBundeslaenderData = () => {
    if (!dailyRawData?.length) return [];
    ...
    const year = state.filters.year;
    if (filteredData[0]?.year !== undefined) {
        filteredData = filteredData.filter(row => row.year == year);
    }

    if (state.filters.agentur !== 'alle' && filteredData[0]?.vermittler_id !== undefined) {
        filteredData = filteredData.filter(row => row.vermittler_id === state.filters.agentur);
    }

    if (state.filters.silo !== 'alle' && filteredData[0]?.silo !== undefined) {
        filteredData = filteredData.filter(row => row.silo === state.filters.silo);
    }

    if (!state.filters.segments.includes('alle') && filteredData[0]?.segment !== undefined) {
        filteredData = filteredData.filter(row => state.filters.segments.includes(row.segment));
    }

    if (!state.filters.products.includes('alle') && filteredData[0]?.product !== undefined) {
    ...
    const result = Object.values(bundeslandMap).map(data => ({
        ...
        bestand: Object.values(data.bestand).reduce((sum, val) => sum + val, 0),
...
export const getLandkreiseData = () => {
    if (!dailyRawData?.length) return [];
...
export const updateAgenturFilterDisplay = () => {
