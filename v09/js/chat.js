...
// ⚠️ CONFIGURATION - Füge hier deinen OpenAI API Key ein
// IMPORTANT: Do NOT commit real API keys to version control!
const OPENAI_API_KEY = 'YOUR_OPENAI_API_KEY_HERE'; // Ersetze mit deinem echten Key
const USE_MOCK_MODE = true; // Setze auf false wenn API-Key konfiguriert ist
...
function initChat() {
    ...
    // Show chat toggle when CSV is loaded
    if (dailyRawData && dailyRawData.length > 0) {
        ...
        if (USE_MOCK_MODE) {
            addMessage('assistant', '⚠️ **Mock-Modus aktiv** - Für echte KI-Antworten konfiguriere deinen OpenAI API-Key in `js/chat.js`.\n\nStelle trotzdem gerne Fragen - ich zeige dir wie die Integration funktioniert!');
        }
    } else {
        console.warn('⚠️ Keine CSV Daten - Chat wird nicht aktiviert');
        return;
    }
